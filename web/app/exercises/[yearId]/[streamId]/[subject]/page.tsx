...
export default function ExercisesPage({ params }: { params: { yearId: string; streamId: string; subject: string } }) {
  ...
  useEffect(() => {
    fetchExercises()
  }, [params.yearId, params.streamId, params.subject, userRole])

  const fetchExercises = async () => {
    setIsLoading(true)
    try {
      const response = await fetch(
        `/api/exercises?yearId=${params.yearId}&streamId=${params.streamId}&subject=${params.subject}${userRole ? `&userRole=${userRole}` : ""}`,
      )
      if (!response.ok) {
  ...
  const handleDeleteExercise = async (id: number) => {
    ...
    try {
      const response = await fetch(
        `/api/exercises?id=${id}&yearId=${params.yearId}&streamId=${params.streamId}&subject=${params.subject}`,
        { method: "DELETE" },
      )
      if (!response.ok) {
