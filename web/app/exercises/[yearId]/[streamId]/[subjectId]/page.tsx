...
export default function ExercisesPage({ params }: { params: { yearId: string; streamId: string; subjectId: string } }) {
  ...
  useEffect(() => {
    fetchExercises()
  }, [params.yearId, params.streamId, params.subjectId, userRole])

  const fetchExercises = async () => {
    setIsLoading(true)
    try {
      const response = await fetch(
        `/api/exercises?yearId=${params.yearId}&streamId=${params.streamId}&subjectId=${params.subjectId}${userRole ? `&userRole=${userRole}` : ""}`,
      )
      if (!response.ok) {
  ...
  const handleDeleteExercise = async (id: number) => {
    ...
    try {
      const response = await fetch(
        `/api/exercises?id=${id}&yearId=${params.yearId}&streamId=${params.streamId}&subjectId=${params.subjectId}`,
        { method: "DELETE" },
      )
      if (!response.ok) {
  ...
  const handleApproveExercise = async (id: number) => {
    ...
    try {
      ...
      setExercises(exercises.map((exercise) => (exercise.id === id ? { ...exercise, approved: true } : exercise)))
      toast({
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white">
      ...
      <main className="container mx-auto px-4 py-16">
        ...
        >
          <h1 className="text-4xl font-bold text-blue-700 mb-2">قسم التمارين والفروض</h1>
          <h2 className="text-2xl font-semibold text-blue-600">
            {decodeURIComponent(params.subjectId)} - السنة {params.yearId} ثانوي -{" "}
            {params.streamId === "literary" ? "أدبي" : "علمي"}
          </h2>
        </motion.div>
        {isLoading ? (
                  ...
                  <Card className="transition-all duration-300 ease-in-out transform hover:scale-105 border-blue-200 hover:border-blue-400 hover:shadow-lg">
                    ...
                    <CardContent className="pt-4">
                      ...
                      <div className="text-sm text-gray-600 mb-4">
                        الشعبة: {exercise.streamId === "literary" ? "أدبي" : "علمي"}
                      </div>
                      <Button
          ...
          >
            <p className="text-xl text-blue-600 mb-4">لا توجد تمارين متاحة حالياً لهذه المادة والسنة الدراسية والشعبة</p>
          </motion.div>
        )}
      </main>
    </div>
